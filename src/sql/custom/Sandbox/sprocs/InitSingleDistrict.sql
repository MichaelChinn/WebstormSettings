...
*/
    ...
    WHERE   schoolcode <> ''
            AND districtCode = @pDestDistrict
            AND ( districtschoolname LIKE '%School 1'
                  OR districtschoolname LIKE '%School 2'
                  OR districtschoolname LIKE '%School 3'
                  OR districtschoolname LIKE '%School 4'
                  OR districtschoolname LIKE '%School 5'
                )
	...
	-- this must happen after all the teachers are inserted but before the assignment so that the seevaluation record is created for all the users
	...
	-- this must happen after all the principals are inserted but before the assignment so that the seevaluation record is created for all the users
