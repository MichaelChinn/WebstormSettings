...
where id = object_id('dbo.GetIndividualScoringDataDetail') and sysstat & 0xf = 4)
   BEGIN
      PRINT '.. Dropping sproc GetIndividualScoringDataDetail.'
      drop procedure dbo.GetIndividualScoringDataDetail
   END
GO
PRINT '.. Creating sproc GetIndividualScoringDataDetail.'
GO
CREATE PROCEDURE dbo.GetIndividualScoringDataDetail
...
SELECT rr.RubricRowID
 ...
 WHERE rrs_non_anchor_score.RubricRowID=rrs_anchor_score.RubricRowID
   ...
   AND rrs_non_anchor_score.SEUserID=@pSEUserID
...
-- adjacent-low by criteria
SELECT DISTINCT x.NonAnchorUserID AS SEUserID, x.StateCriteria, COUNT(*) As Count
  FROM dbo.#RubricRowScores x
 WHERE (x.AnchorScore-x.NonAnchorScore)=1
 GROUP BY x.NonAnchorUserID, x.StateCriteria

-- adjacent-high by criteria
SELECT DISTINCT x.NonAnchorUserID AS SEUserID, x.StateCriteria, COUNT(*) As Count
  FROM dbo.#RubricRowScores x
