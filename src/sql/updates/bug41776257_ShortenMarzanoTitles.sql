...
select @bugFixed = 41776257
, @title = 'bug41776257_ShortenMarzanoTitles'
, @comment = 'Schema change only; just add the new column'
/* ***UPDATE ME*** ***UPDATE ME*** ***UPDATE ME*** ***UPDATE ME*** ***UPDATE ME*** */
/***********************************************************************************/
/*  Notes...
...
[TitleToolTip] [varchar] (MAX) COLLATE SQL_Latin1_General_CP1_CI_AS NULL

SELECT @sql_error = @@ERROR
IF @sql_error <> 0
BEGIN
	SELECT @sql_error_message = 'Failed to add columns. In: ' 
		+ @bugFixed
