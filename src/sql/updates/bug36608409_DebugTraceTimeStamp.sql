...
select @bugFixed = 36608409
, @title = 'bug36608409_DebugTraceTimeStamp.sql'
, @comment = ''
...
/***** VVVV                  VVVVVV ************************************************/
	...
	ALTER TABLE dbo.debugTrace ADD TimeStamp DateTime NULL

	SELECT @sql_error = @@ERROR
	IF @sql_error <> 0
	BEGIN
		SELECT @sql_error_message = 'Failed to add timestamp to debugTrace. In: ' 
