...
select @thisVersion = 5
, @title = 'Add RRAnnotationsVisible field to SEFinalScore'
/* ***UPDATE ME*** ***UPDATE ME*** ***UPDATE ME*** ***UPDATE ME*** ***UPDATE ME*** */
/***********************************************************************************/
/*  Notes...
...
if not exists (select column_name from INFORMATION_SCHEMA.columns where table_name = 'SEFinalScore' and column_name = 'RRAnnotationsVisible') 
BEGIN

ALTER TABLE dbo.SEFinalScore  
	ADD RRAnnotationsVisible BIT

SELECT @sql_error = @@ERROR
IF @sql_error <> 0
   BEGIN
	  SELECT @sql_error_message = 'Alter table failed' 
