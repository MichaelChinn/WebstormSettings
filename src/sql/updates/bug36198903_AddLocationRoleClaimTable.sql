...
select @bugFixed = 36198903
, @title = 'bug36198903_AddLocationRoleCLaimTable'
, @comment = ''
/* ***UPDATE ME*** ***UPDATE ME*** ***UPDATE ME*** ***UPDATE ME*** ***UPDATE ME*** */
/***********************************************************************************/
/*  Notes...
...
CREATE TABLE [dbo].[LocationRoleClaim](
	...
	[LocationRoleClaim] VARCHAR(3000) NOT null
) ON [PRIMARY] 

SELECT @sql_error = @@ERROR
IF @sql_error <> 0
BEGIN
	SELECT @sql_error_message = 'Cannot make new LocationRoleClaim table In: ' 
		+ @bugFixed
