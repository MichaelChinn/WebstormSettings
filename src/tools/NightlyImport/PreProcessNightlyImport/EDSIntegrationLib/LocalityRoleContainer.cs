...
namespace EDSIntegrationLib
{
    public class LocalityRoleContainer
    {
        ...
        public LocalityRoleContainer(SqlDataReader r, Dictionary<string, string> school2District)
        {
            _school2District = school2District;
            while (r.Read())
            {
                List<string> localeRoles = new List<string>();
                string locality = GetStringProperty(r, "CDSOrgCode", "");
                _localityCDSCodes.Add(locality);

                for (int i = 0; i < 10; i++)  // 10 roles max in database!!!
                {
                    string role = (string)GetStringProperty(r, "Role" + (i + 1).ToString(), "");
                    if (role.Length > 0)
                        localeRoles.Add(role);
                }
                ...
                if (!_htCDSCode2OrgName.ContainsKey(locality))
                    _htCDSCode2OrgName.Add(locality, (string)GetStringProperty(r, "CDSOrgName", ""));
            }
        }
