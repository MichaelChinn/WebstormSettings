...
namespace StateEval.tests
{
    ...
    class tClaimsRewrite
    {
        ...
        [Test]
        public void FindSchoolReturn()
        {
            
            SqlParameter[] aParams = new SqlParameter[] { new SqlParameter("@pSchoolCode", "1000") };
            
            string districtCode = (string)Fixture.SEMgr.DbConnector.ExecuteScalar("GetDistrictCodeForSchool", aParams);

            Assert.AreEqual(null, districtCode);

            aParams = new SqlParameter[] { new SqlParameter("@pSchoolCode", "1520") };

            districtCode = (string)Fixture.SEMgr.DbConnector.ExecuteScalar("GetDistrictCodeForSchool", aParams);

            Assert.AreEqual("31015", districtCode);
        }
        private string FakeSchoolCodeInsert(string districtCode, string districtName, string schoolCode, string schoolName)
        {
            _fakeSchoolCodeInsertWasExecuted = true;
            return schoolCode;
        }

        bool _fakeSchoolCodeInsertWasExecuted = false;
        [Test]
        public void FixupLocalityTest()
        {
            //just want to see that the delegate gets hit...
            EDSUserSyncHelper sh = new EDSUserSyncHelper(Fixture._connectionString);
            sh.LocalityFixupFunction = this.FakeSchoolCodeInsert;

            string schoolCode=null;
            string districtCode = null;

            sh.LocalityFixupFunction("10000", "floble dist", "1000", "flooble schoolName");
            Assert.AreEqual(true, _fakeSchoolCodeInsertWasExecuted);
        }
        [Test]
        ...
        public void TestBaseErrors()
        {
            ...
            */
            ...
            LookForException(userName, "unrecognized school", sbRoleString);
            sbRoleString.Clear();
        }
